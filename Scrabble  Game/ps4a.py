    ...
    while len(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if isValidWord(answer, hand, wordList) == False: 
                print('Use letter only in your hand, try again.\n')
                continue    
            # Otherwise (the word is valid):
                ...
                total_score += score
                print(f'"{answer}" earned {score} points. Total: {total_score} points\n')
                # Update the hand 
                hand = updateHand(hand, answer)
                total_answer += answer
                
        if len(total_answer) == n:
...
def playGame(wordList):
    ...
    while True:
        ...
        if mode == 'n':
            ...
            iter += 1
        if mode == 'r':
            # in no hand is played print you haven't played yet
            if iter == 0:
                print('You have not played a hand yet. Please play a new hand first!\n')
                continue
            # if hand is played before
            playHand(hand, wordList, HAND_SIZE)
        if mode == 'e':
            break
